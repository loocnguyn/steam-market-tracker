...
import type { ItemOrders } from "@/lib/steam/types";
...
async function fetchOrders(url: string): Promise<ItemOrders> {
      ...
      {data && (
        <>
          <div className="flex gap-4 text-sm">
            <div>
              <span className="text-zinc-500">Bán thấp nhất: </span>
              <span className="font-medium text-emerald-400">
                {data.lowestSell != null ? fmt.format(data.lowestSell) : "—"}
              </span>
            </div>
            <div>
              <span className="text-zinc-500">Mua cao nhất: </span>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div>
              <p className="mb-1 font-medium text-zinc-400">Lệnh bán</p>
              <OrderTable rows={data.sell.slice(0, 6)} tone="sell" />
            </div>
            <div>
              <p className="mb-1 font-medium text-zinc-400">Lệnh mua</p>
...
function OrderTable({
  ...
  if (rows.length === 0) {
    return <p className="text-zinc-600">Không có dữ liệu</p>;
  }
  return (
    <table className="w-full">
      <thead>
        <tr className="text-zinc-600">
          <th className="text-left font-normal">Giá</th>
